...
export default function ActionButtons({
  ...
  const handleCall = useCallback(() => {
    ...
    trackContact("phone", provider.id);
  ...
  const handleWhatsApp = useCallback(() => {
    ...
    trackContact("whatsapp", provider.id);
  ...
  const handleReport = useCallback(() => {
    const fullName =
      language === "ar"
        ? `${provider.firstnameAr} ${provider.lastnameAr}`.trim()
        : `${provider.firstnameFr} ${provider.lastnameFr}`.trim();
    const reportMessage =
      language === "ar"
        ? `مرحبا، اريد الابلاغ عن مزود خدمة.\nالاسم: ${fullName}\nالهاتف: ${provider.phone}\nالسبب: `
        : `Bonjour, je souhaite signaler un prestataire.\nNom: ${fullName}\nTelephone: ${provider.phone}\nMotif: `;


    window.open(
      `https://wa.me/212601173734?text=${encodeURIComponent(reportMessage)}`,
      "_blank"
    );
  }, [language, provider]);
  ...
  return (
    <div
      className={`action-buttons${compact ? " action-buttons--compact" : ""}`}
    >
      ...
      <button
        className="action-btn action-btn--directions"
      ...
      <p className={`action-disclaimer${compact ? " action-disclaimer--compact" : ""}`}>
        {t("platform_disclaimer")}
      </p>
      <button
        type="button"
        className={`report-link${compact ? " report-link--compact" : ""}`}
